...
async function fetchGithubDiscussionsUnderATopic(repoOwner, repoName, octokit, topicId, cursor = null) {
    ...
    return [response.repository.id, response.repository.discussions];
}
async function fetchAllGithubDiscussionsUnderATopic(repoOwner, repoName, octokit, topicId) {
...
export default async function makeGithubActions(octokit, repoOwner, repoName, categoryId, websiteBase, json) {
    ...
    for (const existingPost of data) {
        ...
        if (index === -1) {
            if (existingPost.locked || existingPost.closed)
                continue;
